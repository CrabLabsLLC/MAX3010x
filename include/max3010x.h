...
/**
 * @brief Lock the device for a multi-register transaction.
 *
 * The MAX30101 exposes PPG and die-temperature controls through the same
 * register bank. Callers that perform a multi-step sequence must hold this
 * lock across the whole sequence.
 *
 * @param[in] dev     Zephyr device handle
 * @param[in] timeout Mutex wait timeout
 * @return 0 on success, negative errno on failure
 */
int max3010xLock(const struct device* const dev, const k_timeout_t timeout);

/**
 * @brief Unlock the device after max3010xLock().
 *
 * @param[in] dev Zephyr device handle
 */
 ...
 * Configuration returns to POR defaults; call max3010xApplyDefaultConfig()
 * or configure the device explicitly before enabling measurements.
 *
 * @param[in] dev Zephyr device handle
 ...
 *
 * MAX30101 uses both green and green2 for its green LED. MAX30102 rejects
 * nonzero green or green2 amplitudes because registers 0x0E and 0x0F are
 * reserved on that variant.
 *
 * @param[in] dev    Zephyr device handle
 ...
 *
 * Valid slot values are disabled, red, IR, and green. Enabled slots must be
 * contiguous from slot 1. Green is rejected on MAX30102.
 *
 * @param[in] dev    Zephyr device handle
...
/**
 * @brief Read and decode the current device configuration.
 *
 * The returned values come directly from the MAX3010x registers rather than
 * cached driver state. Callers must serialize this multi-register snapshot
 * with other device transactions when concurrent access is possible.
 *
 * @param[in]  dev     Zephyr device handle
 * @param[out] config  Destination runtime configuration
 * @return 0 on success, negative errno on failure
 */
 ...
 *
 * The read size must contain complete samples for the configured active
 * slots.
 *
 * @param[in]  dev        Zephyr device handle
 ...
 *
 * Reads FIFO write and read pointers and computes the difference. Equal
 * 5-bit pointers are inherently ambiguous: they can mean empty or exactly
 * 32 unread samples. The driver combines the pointers with the overflow
 * counter and activity observed through prior FIFO/status reads. Servicing
 * the FIFO from PPG_RDY or A_FULL before saturation remains required for a
 * provably lossless stream.
 *
 * @param[in]  dev   Zephyr device handle
