...
typedef enum
{
	...
	MAX3010X_SLOT_GREEN_LED3_PA = 3, ///< Green LED (MAX30101 only, LED3_PA + LED4_PA)
} MAX3010xSlot;
...
/** @brief Decoded runtime configuration read directly from device registers */
typedef struct
{
	MAX3010xMode mode; ///< Active mode selection
	MAX3010xFIFOConfig fifo; ///< FIFO configuration
	MAX3010xSPO2Config spo2; ///< ADC range, sample rate, and pulse width
	MAX3010xSlotConfig slots; ///< Multi-LED slot sequence
	...
	uint8_t proximity_interrupt_threshold; ///< Proximity threshold register value
	bool is_shutdown; ///< Device is in low-power shutdown
} MAX3010xConfig;
